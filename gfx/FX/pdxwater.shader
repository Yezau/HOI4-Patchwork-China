...
PixelShader =
{
	...
	[[
		...
		float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
		{
			...
			float vSpecularIntensity = 0.0015f;
			...
			float pwcShelf = smoothstep( 0.35f, 0.98f, waterHeight );
            float3 pwcSea = lerp( float3( 0.055f, 0.115f, 0.155f ),
                float3( 0.16f, 0.27f, 0.30f ), pwcShelf );
            refractiveColor = lerp( pwcSea, refractiveColor, 0.12f );
            refractiveColor = lerp( refractiveColor, reflectiveColor, fresnel * 0.035f );
